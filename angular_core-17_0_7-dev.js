...
function ɵɵdefer(index, primaryTmplIndex, dependencyResolverFn, loadingTmplIndex, placeholderTmplIndex, errorTmplIndex, loadingConfigIndex, placeholderConfigIndex, enableTimerScheduling) {
  ...
  populateDehydratedViewsInLContainer(lContainer, tNode, lView);
  const lDetails = [null, DeferBlockInternalState.Initial, null, null, null, null
...
function applyDeferBlockState(newState, lDetails, lContainer, tNode, hostLView) {
  ...
  if (stateTmplIndex !== null) {
    ...
    const embeddedLView = createAndRenderEmbeddedLView(hostLView, tNode2, null, {
      dehydratedView
    });
      ...
      if (tNode.type & 8) {
        ...
        if (tNode.type & 1) {
          ...
          if (rNode.textContent === "") {
            context.corruptedTextNodes.set(rNode, "ngetn"
            /* TextNodeMarker.EmptyNode */);
          } else if (rNode.nextSibling?.nodeType === Node.TEXT_NODE) {
            context.corruptedTextNodes.set(rNode, "ngtns"
